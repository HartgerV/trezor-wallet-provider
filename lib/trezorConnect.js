...
function TrezorConnect() {
  ...
  const LOGIN_ONCLICK = 'TrezorConnect.requestLogin(' + "'@hosticon@','@challenge_hidden@','@challenge_visual@','@callback@'" + ')';

  const LOGIN_HTML = `${'<div id="trezorconnect-wrapper">' + '  <a id="trezorconnect-button" onclick="'}${LOGIN_ONCLICK}">` + '    <span id="trezorconnect-icon"></span>' + '    <span id="trezorconnect-text">@text@</span>' + '  </a>' + '  <span id="trezorconnect-info">' + '    <a id="trezorconnect-infolink" href="https://www.buytrezor.com/"' + '       target="_blank">What is TREZOR?</a>' + '  </span>' + '</div>';
...
function parseHDPath(string) {
  return string.toLowerCase().split('/').filter(p => p !== 'm').map(p => {
    let hardened = false;
    if (p[p.length - 1] === "'") {
      hardened = true;
...
function Popup(url, origin, name, width, height) {
  const left = (screen.width - width) / 2;
  const top = (screen.height - height) / 2;
  const opts = `width=${width},height=${height},left=${left},top=${top},menubar=no` + ',toolbar=no' + ',location=no' + ',personalbar=no' + ',status=no';
